...
function gowilds_render_option_font(){
    $fonts = gowilds_fonts();
    $output = '';
...
function gowilds_typography_font_styles($option, $selectors) {
    $output = $selectors . ' {';
...
function gowilds_typography_enqueue_google_font($font) {
 
    if($font && $font != "---"){
        if(array_search($font, array_keys(gowilds_fonts())) > 17){
...
function gowilds_links_typography_font($json){
    $links_fonts = '';
    $customize = (array)json_decode((string)$json, true);
    if(isset($customize['font_family_primary']) && $customize['font_family_primary']){
        $links_fonts .= gowilds_typography_enqueue_google_font($customize['font_family_primary']);
    }
    if(isset($customize['font_family_second']) && $customize['font_family_second']){
        $links_fonts .= gowilds_typography_enqueue_google_font($customize['font_family_second']);
    }
    return $links_fonts;
}

 ...
 */
function gowilds_form_system_theme_settings_alter(&$form, &$form_state) {
	$form['#attached']['library'][] = 'gowilds/gowilds-admin';  
	...
	$form['general'] = array(
		...
		'#default_value' => theme_get_setting('listing_search_action') ? theme_get_setting('listing_search_action') : 'tours-page'
	); 
	...
	if(count($languagesAll) > 1){
		foreach ($languagesAll as $key => $language) {
			$form['general']['listing_search_action' . $language->getID()] = array(
				...
				'#default_value' => theme_get_setting('listing_search_action'. $language->getID()) ? theme_get_setting('listing_search_action'. $language->getID()) : 'tours-page'
			); 
		}
	}
	...
	$form['header']['default_header'] = array(
		...
		'#options' => array(
			'header' => t('Header 01'),
			'header-2' => t('Header 02'),
			'header-3' => t('Header 03'),
			'header-4' => t('Header 04')
		),
	);
